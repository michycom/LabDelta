...
export function DemoDataWorkspace(props: DemoDataWorkspaceProps) {
  const openInOriginalDocument = (parameterName: string) => {
    document.querySelector<HTMLElement>(`[data-demo-target="original-document"] [data-parameter-key="${parameterAnchorKey(parameterName)}"]`)?.scrollIntoView({ behavior: "smooth", block: "center" });
  };
    ...
    <section className="demo-record-view" aria-label="Selected patient demo records">
      {props.isLoadingPatientData ? <div className="demo-state compact" aria-live="polite">Loading patient details and laboratory reports…</div> : props.patientDataError ? <div className="demo-state compact error-state" role="alert"><strong>Patient data could not be loaded.</strong><span>{props.patientDataError}</span></div> : props.patientDetails ? <>
        ...
        <div className="demo-report-layout">
          ...
          </nav><div className="parameter-history-anchors"><strong>Prepared parameter anchors</strong><p>Stable links for a future coordinated highlight; no additional calculation.</p><div>{props.values.slice(0, 8).map(value => <span data-parameter-key={parameterAnchorKey(value.original.parameterName)} key={value.id}>{value.original.parameterName}</span>)}</div></div></CollapsiblePanel>

          <CollapsiblePanel className="comparison-panel" demoTarget="comparison-table" storageKey="comparison-table" title="Report Values"><div className="demo-report-detail">
            {props.selectedReport ? <>
              ...
              {props.isLoadingValues ? <div className="demo-state compact" aria-live="polite">Loading explicitly confirmed values…</div> : props.valuesError ? <div className="demo-state compact error-state" role="alert"><strong>Confirmed values could not be loaded.</strong><span>{props.valuesError}</span></div> : props.values.length === 0 ? <div className="demo-state compact"><strong>No explicitly confirmed working values are stored for this report.</strong></div> : <div className="confirmed-values-wrap" data-demo-target="confirmed-values"><table className="confirmed-values"><thead><tr><th>Original parameter</th><th>Confirmed value</th><th>Unit</th><th>Supplied reference</th><th>Provenance / source location</th></tr></thead><tbody>{props.values.map((value, index) => <tr data-demo-target={index === 0 ? "provenance" : undefined} data-parameter-key={parameterAnchorKey(value.original.parameterName)} data-working-value-id={value.id} key={value.id}><td><strong>{value.original.parameterName}</strong><small>Original text: {value.original.valueText}</small></td><td><strong>{confirmedValueText(value)}</strong><small>Explicitly confirmed · version {value.versionNumber}</small></td><td>{value.unit ?? "Not supplied"}</td><td>{value.original.suppliedReferenceRange ?? "Not supplied"}</td><td><span className="provenance-line"><MapPin size={14} /> Source</span><strong>{props.patientDetails?.displayName ?? "Synthetic patient"} – Laboratory Report</strong><small><b>Location</b><br />Laboratory values · {value.original.parameterName}</small><small className="technical-path"><b>Technical path</b><br />{locatorLabel(value.provenance.locator)}</small><small>Remark: {value.provenance.textExcerpt ?? "No excerpt supplied"}</small><button className="open-original-link" onClick={() => openInOriginalDocument(value.original.parameterName)} type="button">Open in Original Document</button></td></tr>)}</tbody></table></div>}
            </> : props.reports.length > 0 ? <div className="demo-state compact">Select a laboratory report.</div> : null}
          </div></CollapsiblePanel>
        </div>
        <CollapsiblePanel demoTarget="original-document" storageKey="original-document" subtitle="Generated from the selected approved synthetic fixture" title="Original Document">
          {props.selectedReport && !props.isLoadingValues && props.values.length > 0 ? <SyntheticDocumentView patient={props.patientDetails} report={props.selectedReport} values={props.values} /> : <p className="inline-empty">Select a report with confirmed synthetic values to generate its document pages.</p>}
        </CollapsiblePanel>
